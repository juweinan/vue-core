...
1. 原始数据发生改变（这个原始数据被计算属性使用了）
2. 触发 `Computed` 的 `Scheduler`（设置 `_dirty` 为 `true`）
3. 触发 `Computed` 的 `trigger`（通知业务 `Effect`，因为业务 `Effect` 监听了它）
4. 业务 `Effect` 重新执行
5. 再次访问 `Computed.value`
6. 发现 `_dirty` 为 `true`
7. 执行 `Computed` 的 `run`（得出新值）